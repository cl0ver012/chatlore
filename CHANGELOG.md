...
- Language model client (`chatlore.llm`) for any OpenAI-compatible chat API. OpenRouter with
  `deepseek/deepseek-v4-flash` is the default; Ollama, vLLM, and other local servers work by
  setting `CHATLORE_LLM_BASE_URL`. `chatlore doctor` shows the configured model and whether a
  key is set. Model guide in `docs/models.md`.
